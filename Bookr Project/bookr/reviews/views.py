...
def publisher_edit(request, pk=None):
    ...
    return render(request, "reviews/instance-form.html", {"method": request.method, "form": form,
                                                          "model_type": "Publisher", "instance": publisher, }, )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'cover' in request.FILES:
                ...
                if cover_image.width > 300 or cover_image.height > 300:
                    img = Image.open(cover_image)
                    img.thumbnail((300, 300), Image.LANCZOS)
                    img.save(cover_image.path)
            book.save()
            messages.success(request, 'Đã thực hiện thành công')
            return redirect("book_detail", book.pk)
    else:
        form = BookMediaForm(instance=book)
    return render(request, 'reviews/instance-form.html', {'form': form, 'model_type': 'Book',
                                                          'instance': book, 'is_file_upload': True})
